...
const returnCelsius = (temp) => {
  return parseInt(Math.round((5 / 9) * (temp - 32)));
};
...
class Home extends Component {
  ...
  fiveDayInformation = () => {
    ...
    if (!fiveDayData) {
      ...
      return Object.values(fiveDayData.data.list).map((res, i) => {
        const dataArray = [];
        const time = "12:00:00";
        dataArray.push(res);
        return dataArray.map((dtList, i) => {
          return dtList.dt_txt.includes(time) ? (
        ...
        <Paper style={{ margin: "4em 2em 2em 2em" }}>
          <Suspense fallback={this.loadingComponent()}>
            <LeafletMap
              ...
              loadingProp={this.loadingComponent()}
              position={[
                fiveDayData.data.city.coord.lat,
                fiveDayData.data.city.coord.lon,
              ]}
              zoom={6}
      ...
      <div className={`${classes.container} ${classes.hero}`}>
        ...
        <Grid container className={classes.forecastContainer}>
          ...
          <Grid
            item
            md={12}
            className={`${classes.forecastGrid} ${classes.gridCenter}`}
          >
...
Home = connect(mapStateToProps, { fiveDayDataForecast, getLocation })(Home);
